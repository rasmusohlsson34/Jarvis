#!/usr/bin/env python3
"""JARVIS Bankterminalen — Avanza API"""
...
def fetch_stock(stock):
    try:
        url = "https://www.avanza.se/_mobile/market/stock/" + stock["id"]
        r = requests.get(url, headers=HEADERS, timeout=10)
        if r.status_code == 200:
            d = r.json()
            price   = float(d.get("lastPrice", 0))
            day_pct = float(d.get("changePercent", 0))
            if price > 0:
                print("  " + stock["ticker"] + ": " + str(price) + " SEK  dag: " + str(day_pct) + "%  [LIVE]")
...
def generate_html(results):
    ...
    # CSS as plain string — no f-string so curly braces are safe
    css = (
        ...
        ".status-dot{width:8px;height:8px;border-radius:50%;background:" + dot
        + ";box-shadow:0 0 6px " + dot + ";flex-shrink:0}\n"
    ...
    html = (
        ...
        "    </table>\n"
        "  </div>\n"
        "  <div class=\"footer\">Källa: Avanza · "
        "Uppdateras var 15 min mån–fre 09:00–17:30</div>\n"
        "</div>\n</body>\n</html>"
    )
